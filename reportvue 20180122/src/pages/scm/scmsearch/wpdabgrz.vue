...
<script>
	...
	export default{
		...
		computed:{
				...mapGetters('wpdabgrz',['DateTime','dishesType','dishesName','BusinessState','goodsAttr','Operator']),
		},
		data(){
			return {
				report_num:'SAAS_BI_2017_165',
				title:'物品档案变更日志',
				frpt_id:0,
				hierarchytype:1,
				DateGridParams:{
		...
		methods:{
			...mapMutations('wpdabgrz',['setexplainText','setReport_num']),
			...mapActions('wpdabgrz',['getdishesTypeData']),
			ParamsBusinessFun(value){
				let goodsAttr = [];
				this.goodsAttr.forEach((v)=>{
					let item = "'"+v+"'";
					goodsAttr.push(item);
				})
				let params = {
...
//				};
			...
			exportFun() {
				...
				param={
					...
					"className":"com.tzx.report.po.scm.dao.impl.GoodsRecordChangeLogDaoImpl",//导出类路径
			...
			BusinessSummaryStatistics(p) {
				...
				let SummaryStatisticsUrl = 'report/GoodsRecordChangeLogRest/goodsRecordChangeLog';
