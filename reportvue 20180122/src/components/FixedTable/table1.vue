<template>
	<div class="datagrid1">
	    	...
	    	style="width: 100%;">
	    		...
	    		<template v-for="(item1, index) in datagridOne.columns">
	    			<el-table-column
	    				...
	    				:sortable="item1.children.length == 0"
	    				:type="item1.type"
	    				:header-align="item1.children.length == 0?'':'center'">
	    					...
	    					<template v-for="(item2, index2) in item1.children">
				    			<el-table-column
				    				...
				    				:sortable="item2.children.length == 0">
				    					<template v-for="(item3, index3) in item2.children">
							    			<el-table-column
							    				...
							    				:sortable="item3.children.length == 0"
							    				:resizable="false">
							    			</el-table-column>
							    		</template>
				    			</el-table-column>
	   ...
	   <div class="pager">	   	
		   	<el-pagination 
		   	  v-if="datagridOne.total > PageSizeOne"
...
<script>
	import table2 from '@/components/FixedTable/table2'
	export default {
		name:'table1',
		components:{
			table2
		...
		methods:{
			...
			CalculatedHeight(){
				...
				let width = $('#app').width();
				//获取参数区域的高度
				let height1 = $('.params').height();
				//获取分页区域的高度(+20是因为它有个margin值)
				let height2 = $('.datagrid1>.pager').height() + 20;
				//获取分页区域的宽度
				...
				$('.datagrid').css('height',height3 + 'px');
				//if(width < maxwidth){
...
.datagrid1 .el-table td,
.datagrid1 .el-table th,
.datagrid1 .el-table td>div.cell{
...
/*.datagrid1>.el-table>.el-table__body-wrapper{
	margin-left: 50px;
}
.datagrid1>.el-table>.el-table__body-wrapper>.el-table__body{
	margin-left: -50px;
}*/
</style>