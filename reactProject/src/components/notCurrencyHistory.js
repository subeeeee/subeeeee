...
import '../sass/notCurrencyHistory.scss'
import { showload } from './common/toast'
import { isIphone, isIphoneX, isShowTitle } from '../common/setToken'
import BaseComponent from './baseComponent'
import { checkResult, GetNotCurrencyHistoryApi } from '../common/api/api'

export default class NotCurrencyHistory extends BaseComponent {
  constructor(props) {
    super(props)
  ...
  getdata = (limit, lastId) => {
    ...
    this.GetNotCurrencyHistoryData(limit, lastId)
  }
   ...
   * @param {*} limit
   * @param {*} lastId
   * @memberof NotCurrencyHistory
   */
  async GetNotCurrencyHistoryData(limit, lastId) {
    // let fundCode = this.props.location.query.fundCode
    showload(true)
    let result = await GetNotCurrencyHistoryApi(
      limit,
      this.props.location.query.fundCode,
      lastId
    )
    showload(false)
  ...
  loadData() {
    ...
    let cHeight = document.documentElement.clientHeight
    console.log(cHeight + sTop, sHeight)
  ...
  render() {
    ...
    console.log(list)
    return (
      <div className="notCurrencyhistory">
        <BackTitle
          titleName="历史净值"
          hasPostion={true}
          ...
          {(list || []).map((items, index) => {
            return (
              <div className="hTd" key={index}>
                ...
                <span>{items.nav == '' ? '--' : items.nav}</span>
                <span>{items.navTotal == '' ? '--' : items.navTotal}</span>
                <span
                  className={
                    items.dayIncrease != ''
                      ? items.dayIncrease != '0.00'
                        ? items.dayIncrease > 0
                          ? 'red'
                          : 'green'
                        : ''
