<template>
  <div class="putInPoint public" >
      ...
      <el-col :span="24" class="table_tr">
        ...
        <el-col :span="9" class="table_td">
          <el-col :span="14" :offset="1">
            ...
            <el-input v-model="amount" :disabled="reCheck" size="small" @blur="count('借款金额')" @change="requiredFields()" :placeholder="'请输入借款金额（例如：123）'"></el-input>
    ...
    <picModule dataType="1" :bussNo="$route.query.bussNo"  taskType="210"  :details="!reCheck" class="module_margin_bottom"/>
    <p class="error_title" v-if="reCheck">以上信息已复核，不可修改。</p><br/>
    <div class="show_list_title">
    ...
    <el-row v-if="isCityShow">
      <el-col :span="9" :offset="4" style="text-align: right">
        预约公证时间：{{notaryAppointFlowVO.appointTime}}
      </el-col>
      <el-col :span="4" :offset="1">
        <el-checkbox  v-model="timestampBol" @change="timeBolFn"></el-checkbox>取消预约
      </el-col>
      <!--  <el-col v-if="timeBol" class="importent" :span="6">
          若不需要取消预约，请不要“提交”
        </el-col>-->
    </el-row>
    <br/>
    ...
    <el-dialog title="提示" :visible.sync="dialogVisible" width="30%">
      <div>
        <el-row style="margin: 0 auto;width:90%;line-height:30px">
          <el-col :span="24" style="margin-bottom: 10px" v-if="isCityShow&&!reCheck">
          ...
          <el-col :span="24" style="margin-bottom: 10px" v-if="isCityShow&&!reCheck">
            ...
            <el-col :span="12" :offset="1" style="text-align: left">{{!timestampBol?notaryAppointFlowVO.appointTime:'取消预约'}}</el-col>
          </el-col>
          <p style="margin-bottom: 10px;text-align: center" v-if="isCityShow&&reCheck">请确认是否取消预约？ </p>
          <p style="margin-bottom: 10px;text-align: center" v-if="!isCityShow">请确认是否提交修改信息？ </p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      timeBolFn(){
        if(this.reCheck){
          this.timestampBol?this.timeBol = false:this.timeBol = true;
        }
      },
      brokerMobileFn(str){
        // brokerMobile
              ...
              if(res.resultCode === '0000'){
                if(res.data.check === '审批结论已修改'){
                  ...
                  if(!this.isCityShow){
                    ...
                    this.timestampBol?appointmentOrCancel = 'cancel':appointmentOrCancel = 'appointment'
                  }
                  let subParams = {
                    appointmentOrCancel,
        ...
        if('submit' === type){
          if(this.isCityShow){
            if(!this.reCheck){
              ...
              this.timeBolFn();
              if(this.timeBol){
                this.$message.error('若不需要取消预约，请不要“提交”');
                return
              }
            }
          }else{
               ...
               if(!this.reCheck){
                 if(this.timestampBol){
                   dataMessage = h('div',null,[
                     h('p', null, `借款金额: ${this.amount}元;`),
                     h('p', null, `借款期限: ${this.term}天;`),
                     h('p', null, `预约公证时间:取消预约`),
    ...
    beforeCreate () {
      ...
      $.post(this._path + '/visaInterview/selectContractPreInfoByBussNo',{bussNo:this.$route.query.bussNo},res=>{
        if(res.resultCode == '0000'){
          ...
          this.reCheck = res.data.isPass == '1'?false:true;
          ...
          this.accMobile = !data.accMobile?'':data.accMobile;
          ...
          if(this.interviewFlowId){
            $.post(this._path + "/notaryDistribution/queryNotaryDistributionFlow",{interviewFlowId:this.interviewFlowId}, res => {
              if(res.resultCode == '0000'){
          ...
          $.post(this._path + '/visaInterview/selectProductInfoByBussNo',{bussNo:this.$route.query.bussNo},res=>{
            if(res.resultCode == '0000'){
              ...
              $.post(this._path + '/visaInterview/queryBussProductInfoByBussNo',{bussNo:this.$route.query.bussNo},res=>{
                if(res.resultCode == '0000'){
                  let lengthstr = res.data.bussProductInfoVoList.length;
                  for(let i = 0;i < lengthstr;i++){
                    ...
                    if(res.data.bussProductInfoVoList[i].proCode == this.proCode){
                      // this.termList = this.proArr[i].termList;
