      ...
      <el-col class="condition_space_between">
        ...
        <el-col :span="6">
          <el-select v-model="taskStatus"   placeholder="请选择" class="select_width"  size='small'>
            ...
            <el-option label="已确认" :value="'90'"> </el-option>
          </el-select>
        </el-col>
      </el-col>
      <el-col class="condition_space_between">
    ...
    <el-table :data="tableData" border stripe class="table">
     ...
     <el-table-column header-align="center" align="center" fixed="right" label="操作" width="140">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="dispose(scope.row)" v-show="scope.row.taskStatus === '待确认'" v-if="currentUserPermissions.indexOf('business:contract:costconfirm:handle') != -1">处理</el-button>
          <el-button type="text" size="small" @click="info(scope.row)" v-if="currentUserPermissions.indexOf('business:contract:costconfirm:query') != -1">详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  export default {
    ...
    methods: {
      dispose (row) {
        $.post(this._path + '/busCostConfirmInfo/toCostConfirmInfo',{bussNo:row.bussNo},res=>{
      ...
      requestFn() {
        ...
        $.post(
          this._path + "/busCostConfirmInfo/costConfirmPageList",
          this.params,
          res => {
            if (res.resultCode == "0000") {
              loading.close();
              this.tableData = res.data.busCostConfirmInfoVos;
