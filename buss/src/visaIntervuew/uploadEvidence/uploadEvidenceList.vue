<template>
  <div class="contractPreparingList public">
    ...
    <el-table :data="tableData" border stripe class="table">
     ...
     <el-table-column header-align="center" align="center" prop="flowTime" label="任务流转时间" min-width="200"></el-table-column>
     ...
     <el-table-column header-align="center" align="center" fixed="right" label="操作" width="180">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="dispose(scope.row)" v-show="scope.row.taskStatus === '待处理'" v-if="currentUserPermissions.indexOf('business:contract:upload:handle') != -1">上传凭证</el-button>
          <el-button type="text" size="small" @click="info(scope.row,'withdraw')" v-show="scope.row.taskStatus === '进行中'" v-if="currentUserPermissions.indexOf('business:contract:upload:handle') != -1">撤回</el-button>
          <el-button type="text" size="small" @click="info(scope.row,'info')" v-if="currentUserPermissions.indexOf('business:contract:upload:query') != -1">详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  export default {
    name:'uploadEvidenceList',
    data() {
      return {
        pdf:require('./../../assets/uploadTrial.pdf'),
    ...
    methods: {
      dispose (row) {
        $.post(this._path + '/uploadCertificateInfo/toUploadCertificate',{bussNo:row.bussNo},res=>{
      ...
      requestFn() {
        ...
        $.post(
          this._path + "/uploadCertificateInfo/uploadCertificatePageList",
          this.params,
          res => {
            if (res.resultCode == "0000") {
              loading.close();
              this.tableData = res.data.busUploadCertificateVos;
