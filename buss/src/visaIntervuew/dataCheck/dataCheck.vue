...
<div class="contractPreparingList public">
    <h2>资料核查</h2>
      ...
      <br /><br />
      <div class="show_list_title">
        <h3><img src="./../../assets/task.png" class="listTitleImg"><span>原件核验</span><el-checkbox size="small" class="dataChackSmall" v-model="original"> <span class="font_red">资料齐全且属实</span> </el-checkbox> </h3>
      </div>
      <el-tabs v-model="activeName">
        <el-tab-pane label="借款人" name="1">
          <el-tabs v-model="activeCustName" type="card">
            <el-tab-pane :label="item.custName" :name="index" v-for="(item,index) in borrowMessage">
      ...
      <br />
      <div class="show_list_title">
        <h3><img src="./../../assets/task.png" class="listTitleImg"><span>其他资料清单</span><el-checkbox size="small" class="dataChackSmall" v-model="elseBol"> <span class="font_red">资料齐全且签署无误</span> </el-checkbox> </h3>
      </div>
      ...
      <div class="show_list_title">
        <h3><img src="./../../assets/task.png" class="listTitleImg"><span>当前合同信息</span><el-checkbox size="small" class="dataChackSmall" v-model="contractBol"> <span class="font_red">借款信息已与渠道确认</span> </el-checkbox> </h3>
      </div>
      ...
      <div class="show_list_title">
        <h3><img src="./../../assets/task.png" class="listTitleImg"><span>客户放款银行卡信息</span><el-checkbox size="small" class="dataChackSmall" v-model="accBol"> <span class="font_red">放款卡信息已确认无误</span> </el-checkbox> </h3>
      </div>
      <el-row class="module_margin_bottom tableInfo">
        <el-col :span="24" class="table_tr">
...
export default {
  ...
  name:'contractPreparingList',
	...
	methods: {
		btnClick () {
      if(!this.original){
        this.$message.error('请勾选原件核验核查结果');
        return
      }
      if(!this.elseBol){
        this.$message.error('请勾选其他资料清单核查结果');
        return
      }
      if(!this.contractBol){
        this.$message.error('请勾选当前合同信息核查结果');
        return
      }
      if(!this.accBol){
        this.$message.error('请勾选客户放款银行卡信息核查结果');
        return
      }
      this.$confirm('是否确认提交', '提示', {
        ...
        let params = {
          ...
          loanAmount:this.tableData.amount,
          term:this.tableData.term,
          proCode:this.tableData.proCode,
          accName:this.accName,
          account:this.account,
          accBank:this.accBank,
          accMobile:this.accMobile,
         /* remark:this.remark,
          material:JSON.stringify(this.inventory)*/
        };
        ...
        $.post(this._path +'/verification/verifySubmit',params,res=>{
          loading.close();
          if(res.code == "SUCCESS"){
            this.$message({
              type: 'success',
    ...
    checkoutFn(item,index){
      if(item.name == '资料清单'){
		    this.inventory.forEach((obj)=>{
		      obj.isChoose = item.isChoose
        })
      }else{
        if(!item.isChoose){
          this.inventory[0].isChoose?this.inventory[0].isChoose = false:this.inventory[0].isChoose
        }else{
          let bol = false;
          this.inventory.forEach((obj)=>{
            if(obj.name != '资料清单'&&!obj.isChoose){
              bol = true
            }
          });
          if(!bol){
            this.inventory[0].isChoose = true
          }
        }
      }
    }
