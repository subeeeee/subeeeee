...
<template>
  <div class="firstCheckList public">
    ...
    <el-row>
      <el-col :offset='21' :span='2' class="inquireBtn_warp"><el-button size="small" class="inquireBtn" @click="download">导出台账报表</el-button></el-col>
    </el-row>
    <!-- 列表展示 -->
    <div class="show_list_title">
    ...
    <el-table :data="tableData" border stripe class="table">
      ...
      <el-table-column header-align="center" align="center" fixed="right" label="操作" width="140">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="btnClick(scope.row,'处理')" v-show="scope.row.taskStatus != '已完成'" v-if="currentUserPermissions.indexOf('business:loan:confirm:handle') != -1">处理</el-button>
          <el-button type="text" size="small" @click="btnClick(scope.row,'详情')" v-if="currentUserPermissions.indexOf('business:loan:confirm:query') != -1">详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页按钮 -->
  ...
  export default {
    ...
    methods: {
      download(){
        ...
        this.downloadPackage(this._path + '/loanConfirm/outExcelLedger',params);
      },
      productsListFn(){
      ...
      btnClick(row, type) {
        let path = "";
        if ("处理" == type) {
          $.post(this._path + '/loanConfirm/toLoanConfirm',{bussNo:row.bussNo},res=>{
            if(res.resultCode == '0000'){
              this.$router.push({
                path:'/loanAffirm',
                query: {
      ...
      ajax() {
        $.post(
          this._path + "/loanConfirm/loanConfirmPageList",
          this.params,
          res => {
            if (res.resultCode == "0000") {
              this.tableData = res.data.loanConfirmListVoList;
    ...
    beforeCreate() {
      $.post(this._path + "/loanConfirm/loanConfirmPageList",{},res => {
          if (res.resultCode == "0000") {
            this.tableData = res.data.loanConfirmListVoList;
            this.page.totalCount = res.data.totalCount;
          }else{
