...
<template>
  <div class="firstCheckList public">
    ...
    <el-table :data="tableData" border stripe class="table">
     ...
     <el-table-column header-align="center" align="center" fixed="right" label="操作" width="250">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="btnClick(scope.row,'处理')" v-show="scope.row.taskStatus != '已完成'&& scope.row.taskStatus != '已通过' && scope.row.taskStatus != '已拒绝'" v-if="currentUserPermissions.indexOf('business:loan:finance:handle') != -1">处理</el-button>
          <el-button type="text" size="small" @click="btnClick(scope.row,'详情')" v-if="currentUserPermissions.indexOf('business:loan:finance:query') != -1">详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页按钮 -->
  ...
  export default {
    ...
    methods: {
      ...
      btnClick(row, type) {
        let path = "";
        if ("处理" == type) {
          $.post(this._path + '/loanFinance/toLoanFinance',{bussNo:row.bussNo},res=>{
            if(res.resultCode == '0000'){
              this.$router.push({
                path:'/loan',
                query: {
      ...
      ajax() {
        $.post(
          this._path + "/loanFinance/loanFinancePageList",
          this.params,
          res => {
            if (res.resultCode == "0000") {
              this.tableData = res.data.loanFinanceListVoList;
    ...
    beforeCreate() {
      $.post(this._path + "/loanFinance/loanFinancePageList",{},res => {
          if (res.resultCode == "0000") {
            this.tableData = res.data.loanFinanceListVoList;
            this.page.totalCount = res.data.totalCount;
          }else{
