<template>
  ...
  <div class="public">
      ...
      <el-col class="condition_space_between">
        ...
        <el-col :span="6">
          <el-select v-model="taskStatus" placeholder="请选择" @change="productsListFn" class="select_width"  size='small'>
            ...
            <el-option :label="'已打回'" :value="'30'"></el-option>
            <el-option :label="'未上传'" :value="'60'"></el-option>
    ...
    <el-table :data="tableData" border stripe class="table">
      ...
      <el-table-column header-align="center" align="center" prop="orgName" label="渠道简称" min-width="150"></el-table-column>
      ...
      <el-table-column header-align="center" align="center" prop="taskPerName" label="任务员" min-width="150"></el-table-column>
      ...
      <el-table-column header-align="center" align="center" fixed="right" label="操作" min-width="140">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="btnClick(scope.row,'处理')" v-if="scope.row.taskStatus === '待确认' && currentUserPermissions.indexOf('business:callback:confirm:handle') !== -1">处理</el-button>
          <el-button type="text" size="small" @click="btnClick(scope.row,'详情')" v-if="currentUserPermissions.indexOf('business:callback:confirm:query') !== -1">详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页按钮 -->
...
<script>
  export default {
    name: "first-call-back-affirm-list",
    data(){
      return{
        bussNo:'',
        orgName:'',
    ...
    methods:{
      btnClick(row, type) {
        let path = '';
        if('处理' === type){
          path = '/firstCallBackAffirm';
          this.$http({
            url:'/busLoanCallbackConfirm/dealCallbackConfirm',
            data:{
              bussNo:row.bussNo
            },
            callback:res=>{
              this.$router.push({
                ...
                query:{
                  bussNo:row.bussNo,
                  callbackResult:row.callbackResult,
                  refuseDetail:row.refuseDetail,
      ...
      ajax() {
        this.$http({
          url:'/busLoanCallbackConfirm/busLoanCallbackConfirmPageList',
          data:this.params,
          callback:res=>{
            this.tableData = res.data.busLoanCallbackConfirmVoList;
