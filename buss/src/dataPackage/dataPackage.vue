    ...
    methods: {
      ...
      remove(node, data) {
        this.$confirm("确认删除该资料表", "提示", {
          ...
          .then(() => {
              $.post(this._path+'/package/delete',{id:data.value},(res)=>{
                if(res.resultCode == '0000'){
                     $.get(this._path + "/package/tree",{},res=>{
                        this.listData = res.data.tree;
                      })
                  }else{
      ...
      renderContent(h, { node, data, store }) {
        ...
        if(1){
            add =  <el-button size="mini" on-click={ () => this.append(node, data) }>添加</el-button>;
        }
        if(1){
